...
📄 **Пояснительная записка (курсовой проект)** — [Kursovaya.docx](Kursovaya.docx)
---
...
|-----------|-------------------|---------|-----------|
| backend.services | 20 | 19 | 1* |
| backend.users | 6 | 6 | 0 |
| **ИТОГО** | **26** | **25** | **1** |

> \* Один тест проверяет автоматическую генерацию slug. Ожидалось латинское написание, но Django генерирует русское (например, `составление-договора` вместо `sostavlenie-dogovora`). Это не влияет на работу приложения и может быть исправлено заменой ожидаемого значения в тесте.
...
| Создание заявки с авторизацией | ✅ |
...
| Вход с неверными данными | ✅ |
| Создание заявки без авторизации | ✅ |
| Создание заявки с коротким описанием | ✅ |
| Принятие заявки адвокатом | ✅ |
| Отклонение заявки адвокатом | ✅ |
